...
class SimpleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if not self.isTrain or opt.continue_train:
            self.load_network(self.netG_A, None, opt.saved_model_path)
    ...
    def test(self):
        ...
        self.ground_B = self.input_B.permute(0,3,1,2)
        self.var_mask = self.input_mask.permute(0,3,1,2)
        self.netG_A.eval()
        self.fake_B = self.ground_B.clone()
        self.fake_B.data.zero_()

        print('mask: ', self.var_mask.size())
        ...
        print('real_A_input: ', self.real_A_input.size())
        print('real_A_input: ', self.real_A_input.size())
        print('ground_B: ', self.ground_B.size())
        with torch.no_grad():
            out = self.netG_A.forward(self.real_A_input.permute(0,3,1,2))
        print('out: ', out.size())

        self.fake_B = out #self.fake_B.permute(0,3,1,2)
        #self.fake_B[:,:,:(self.real_A.size(1)//16)*16,:(self.real_A.size(2)//16)*16] = out
        print('after fake_B: ', self.fake_B.size())
    ...
    def backward_G(self):
        ...
        if self.var_mask.data.sum()>0:
            self.fake_B = self.ground_B.clone()
            self.fake_B.data.zero_()
            print('out: ', self.netG_A.forward(self.real_A.permute(0,3,1,2)).size())
            print('real_A: ', self.real_A.size())
            print('fake_B: ', self.fake_B.size())
            self.fake_B[:,:,:,:] = self.netG_A.forward(self.real_A.permute(0,3,1,2)).permute(0,2,3,1)
           # print('out: ', self.netG_A.forward(self.real_A.unsqueeze(1)).size())
            '''
            if self.multiloss:
    ...
    def optimize_parameters(self, i):
        # forward. wrap inputs into variables.
        self.forward()
        # G_A        
       # reset gradients
        for optimizer in self.optimizers:
    ...
    def save(self, label):
        self.save_network(self.netG_A, self.optimizers[0], 'G_A', label, self.gpu_ids)

    def load(self):
        for optimizer in self.optimizers:
 #           print('%%%%%%%%%%%%%%%: ', optimizer)
             self.optimizer_to_load = optimizer
#             i = i+1
#             print('numberOfOptimizers: ', i)
        self.load_network(self.netG_A, self.optimizers[0], self.resume_epoch)
