...
class BaseModel():
    ...
    def save_network(self, network, optimizer, network_label, epoch_label, gpu_ids):
        save_filename = '%s_net_%s.pth' % (epoch_label, network_label)
        save_path = os.path.join(self.save_dir, save_filename)
        torch.save({'epoch': epoch_label, 'state_dict': network.cpu().state_dict(), 'opt_dict': optimizer.state_dict()}, save_path)
        if len(gpu_ids) and torch.cuda.is_available():
            network.cuda(device=gpu_ids[0])
   

    # helper loading function that can be used by subclasses
    def load_network(self, network, optimizer, resume_epoch):
        # save_filename = '%s_net_%s.pth' % (epoch_label, network_label)
        # save_path = os.path.join(self.save_dir, save_filename)
        checkpoint = torch.load(resume_epoch)
