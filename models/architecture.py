...
class Unet_struc(nn.Module):
    def __init__(self, opt, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, gpu_ids=[]):
...
class ST_Attblock(nn.Module):
    def __init__(self, num_channels):
        super(ST_Attblock, self).__init__()
...
class Unet_3ds_STAtt(nn.Module):
    def __init__(self, opt, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, gpu_ids=[]):
        super(Unet_3ds_STAtt, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        self.gpu_ids = gpu_ids
        ...
        self.tmp_pool = nn.AdaptiveAvgPool3d((None, None, 1))
    ...
    def forward(self, input):
        input = input.permute(0,2,3,1).unsqueeze(1)
        print('UNet input: ', input.size())
        ...
        return self.tmp_pool(self.up1(fu1)).squeeze(-1)
...
class Unet_3ds_rcab(nn.Module):
    def __init__(self, opt, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, gpu_ids=[]):
        ...
        model['up1'] = [
            ...
            self.make_rcab(ngf, n_rcab),
            nn.ReLU(True),
            nn.Conv3d(self.ngf, self.output_nc, kernel_size=3, padding=1)
            # norm_layer(self.output_nc),
            # nn.ReLU(True)
            ]
        self.up1 = nn.Sequential(*model['up1'])
...
class Unet_3ds(nn.Module):
    ...
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Multiples GPUs!!')
            return nn.parallel.data_parallel(self.model, input, [0,1,2])
        else:
            return self.model(input)
...
class Net_1by1_struc(nn.Module):
    def __init__(self, opt, temporal_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, gpu_ids=[]):
        super(Net_1by1_struc, self).__init__()
        self.input_nc = temporal_nc
        self.output_nc = output_nc
        self.ngf = ngf
        self.gpu_ids = gpu_ids
        ...
        nn.BatchNorm2d(self.ngf[0]) if norm_layer else nn.LeakyReLU(1),
        nn.ReLU(True)
        ]
        # Originally 4 norm_layer
        ...
        for k in range(opt.FNN_depth-1):
            ...
            model.append(nn.BatchNorm2d(self.ngf[k+1]) if norm_layer else nn.LeakyReLU(1))
            model.append(nn.ReLU(True))
...
#       input = input.permute(0,3,1,2)
       print('FNN input: ', input.size())
       print('FNN output: ', self.model(input).size())
       return self.model(input)
...
class SQ_module(nn.Module):
    def __init__(self, opt, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, gpu_ids=[]):
        ...
        elif opt.Unet_struc == '3dsAtt':
            Post_T1 = Unet_3ds_STAtt(opt, num_D, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
            Post_T2 = Unet_3ds_STAtt(opt, num_D, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
...
class UniNet_init(nn.Module):
    def __init__(self, opt, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, gpu_ids=[]):
        ...
        Pre_T1 = Net_1by1_struc(opt, opt.temporal_nc, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
        Pre_T2 = Net_1by1_struc(opt, opt.temporal_nc, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
        Pre_T1 = Pre_T1.model
        Pre_T2 = Pre_T2.model
        ...
        Post_T1 = get_Unet_model(opt, input_nc, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
        Post_T2 = get_Unet_model(opt, input_nc, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
        
        self.model_T1 = nn.Sequential(Pre_T1,Post_T1)
        self.model_T2 = nn.Sequential(Pre_T2,Post_T2)

    def forward(self, input):
#        print('UniNet input: ', input.size())
        T1 = self.model_T1(input)
#        print('FNN T1: ', T1.size())
        T2 = self.model_T2(input)
        return torch.cat([T1, T2], 1)
...
def get_Unet_model(opt, input_nc, output_nc, ngf, norm_layer, use_dropout, gpu_ids):
    if opt.Unet_struc == '0ds':
        ...
        Unet = Unet_3ds_STAtt(opt, input_nc, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
    elif opt.Unet_struc == '4ds':
...
class UniNet_struc(nn.Module):
    def __init__(self, opt, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, gpu_ids=[]):
        ...
        if opt.Unet_struc == '0ds':
            ...
            Post_T1 = Unet_3ds_STAtt(opt, Pre_T1[-3].out_channels, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
            Post_T2 = Unet_3ds_STAtt(opt, Pre_T2[-3].out_channels, output_nc, ngf, norm_layer, use_dropout, gpu_ids)
        elif opt.Unet_struc == '4ds':
        ...
        self.model_T1 = nn.Sequential(Post_T1) #Pre_T1,Post_T1)
        self.model_T2 = nn.Sequential(Post_T2) #Pre_T2,Post_T2)
