...
parser.add_argument('--n_Network', type=int, default=1, help='for multiband dataset')
parser.add_argument('--accumulation_steps', type=int, default=8, help='gradient accumulation')
parser.add_argument('--temporal_nc', type=int, default=int(2304/4*2), help='temporal channels')
...
parser.add_argument('--resume_epoch', type=str, default='R2Plus1D/380_net_G_A.pth')
...
if opt.epoch_count > 1:
   opt.resume_epoch = opt.checkpoints_dir + '/' + str(opt.epoch_count) + '_net_G_A.pth'
   opt.resume = True
else:
   opt.resume = False

if opt.resume:
   if os.path.isfile(opt.resume_epoch):
      print("=> Loading checkpoint '{}'".format(opt.resume_epoch))
      model.load()
   else:
