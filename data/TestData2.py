...
class MRFDataset(BaseDataset):
    ...
    def get_paths(self):
        if self.opt.onMAC:
            ...
            d_root = '/shenlab/lab_stor6/yilinliu/multiband/MB3/Raw_2304_slice2/Raw_2304/'
        ...
        if self.opt.set_type == 'train':
           for j in range(7):
            ...
            for j in range(11):
                self.data_paths.append({
                'imMRF': d_root+'data'+'/'+a[10],
                'Tmap': d_root+'goals'+'/'+a[10], # sparse dict
                # 'Tmap': d_root+person_path[person[i]-1]+'/'+a[j]+'/patternmatching_densedict.mat', # dense dict
                'mask': d_root+'goals'+'/'+a[10] # large mask
                # 'mask': d_root+'Data_Qian_skull_h5/'+str(person[i])+'/'+str(j+1)+'-skull.mat' # small mask
                })
